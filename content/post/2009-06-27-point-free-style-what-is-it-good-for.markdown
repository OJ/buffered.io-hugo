...
If you're not interested in what inspired this post, then skip this section and <a href="#MoreInteresting">jump</a> to the more interesting bits.

A Little Bit of History
-----------------------

Recently I've been delving into [Haskell](http://haskell.org/) quite a bit. It's part of my apparently never-ending quest to learn as much as I can about as many languages as I can (well, those that appeal to me at least :)). While I love playing around with a language, toying with ideas, writing small programs, reading books, blog posts, etc it's not really the same as having an on-call expert to help and guide you.

<!--more-->

 While I'm aware of, and frequently visit, the [Haskell IRC channell](http://www.haskell.org/haskellwiki/IRC_channel) I find that the level of understanding there is so great that my piddly noob questions tend to get lost amongst the bombardment of much more advanced & interesting discussions. In short, while I find it a great place to go, it's not a great place to find someone who's happy to help guide me through the maze and go over topics in quite a bit of detail while I annoy them with questions.

Haskell is one of those languages where having a mentor is really beneficial. So I set about finding myself one. Thankfully, I found a rather helpful chap based in the UK, <del>(he shall remain anonymous, as I don't want to violate the man's privacy)</del>, [Peter Marks](http://twitter.com/peter_marks), who has been very forthcoming with information. He's humoured me and been incredibly patient so far, and I'm very grateful for his time.

One of the questions that I asked him was: Why is it that everyone seems to strive to get their code into [Point-Free](http://www.haskell.org/haskellwiki/Pointfree) style? I can see how a lot of the implementations are more concise, but many of those lose readability. What is so special about it?
...
If we take a look at the [Wikipedia](http://en.wikipedia.org/wiki/Point-free_programming) we can see that..
...
Haskell developers use the [composition operator](http://www.haskell.org/haskellwiki/Function_composition) [(.)](http://haskell.org/ghc/docs/latest/html/libraries/base/src/GHC-Base.html#.) a lot. It actually aids in creating Point-Free style. I love the irony here. We add points (.) to remove points (arguments).

Anyway, the composition operator's definition, according to [Prelude](http://www.haskell.org/onlinereport/standard-prelude.html) is:
``` haskell
(.)       :: (b -> c) -> (a -> b) -> a -> c
(.) f g x = f (g x)
```
...
That is:
* It takes an argument which is a function which takes a value of type `b` which returns a value of type `c`.
* It takes a function which takes a value of type `a` which returns returns a value of type `b`.
* It returns a new function which takes a value of type `a` and returns a value of type `c.
